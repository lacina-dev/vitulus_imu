...
class BNO08X:  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def calibration_status(self):
        ...
        return self._magnetometer_accuracy, self._accelerometer_accuracy, self._gyro_accuracy, self._game_accuracy
    ...
    def _process_available_packets(self, max_packets=None):
        ...
        while self._data_ready:
            ...
            try:
                ...
                continue
            # print("Processedaa", processed_count, "packets")
            self._handle_packet(new_packet)
            processed_count += 1
            self._dbg("")
            # print("Processeda", processed_count, "packets")
            self._dbg("")
        self._dbg("")
        self._dbg(" ** DONE! **")
    ...
    def _process_report(self, report_id, report_bytes):
        ...
        if report_id == BNO_REPORT_GYROSCOPE:
            self._gyro_accuracy = accuracy
        if report_id == BNO_REPORT_GAME_ROTATION_VECTOR:
            self._game_accuracy = accuracy
